...
void OWI_TransmitData(uint8_t * data, uint8_t dataLength);
void OWI_ReceiveData(uint8_t * data, uint8_t dataLength);
...
typedef union 
{
    uint8_t allFlags; //!< Variable used to clear all flags.
    struct
    {
        uint8_t busy:1; //!< 1 bit wide bitfield used to signal that the 1-Wire driver i busy.
        uint8_t presenceDetected:1; //!< 1 bit wide bitfield used to reflect whether presence was detected.
        uint8_t error:1; //!< 1 bit wide bitfield used to flag error.
    };
} OWIflags;
