...
#define BUTTON1_MASK (1 << PA1)
#define BUTTON2_MASK (1 << PA4)
#define BUTTON3_MASK (1 << PA0)
#define BUTTON4_MASK (1 << PA2)
#define BUTTON5_MASK (1 << PA5)

#define BUTTON_MASK  (BUTTON1_MASK | BUTTON2_MASK | BUTTON3_MASK | BUTTON4_MASK | BUTTON5_MASK)

// Variable to tell that the button is pressed (and debounced).  Can be read with button_down() which will clear it.
...
// Decrease 2 bit vertical counter where mask = 1.  Set counters to binary 11 where mask = 0.
...
static inline void debounce(void) {
	...
	// Read buttons (active low so invert with ~). Xor with button_state to see which ones are about to change state
